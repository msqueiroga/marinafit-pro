...
function renderDashboard(){const p=db.profile;$('#welcomeName').textContent=p.name||'Marina';const w=db.weights.at(-1)?.value||p.weight;$('#weightStat').textContent=w?`${w} kg`:'—';$('#weightTrend').textContent=db.weights.length>1?'Histórico atualizado':'Registre seu primeiro peso';$('#workoutStat').textContent=db.workouts.length;const a=db.assessments.at(-1);if(a){let d=new Date(a.date);d.setDate(d.getDate()+28);$('#assessmentStat').textContent=d.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'})}else $('#assessmentStat').textContent='—'}
...
function paintAvatar(element,profile){if(!element)return;element.textContent=profile.avatar?'':(profile.name||'M')[0].toUpperCase();element.style.backgroundImage=profile.avatar?`url(${profile.avatar})`:''}
...
if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=76');setTheme(db.theme);initEvents();initDiastasisGuideEvents();renderAll();
